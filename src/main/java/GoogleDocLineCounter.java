...
public class GoogleDocLineCounter {
  ...
  private static Drive createDriveService(String credentialsFilePath)
      throws IOException, GeneralSecurityException {
    GoogleCredential credential =
        GoogleCredential.fromStream(new FileInputStream(credentialsFilePath)).createScoped(SCOPES);
  ...
  private static void downloadGoogleDoc(Drive driveService, String documentId, String outputFile)
    ...
    try (FileOutputStream outputStream = new FileOutputStream(outputFile)) {
      driveService.files().export(documentId, mimeType).executeMediaAndDownloadTo(outputStream);
    }
  }
