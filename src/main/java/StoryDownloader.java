...
public class StoryDownloader {
    ...
    public static void main(String[] args) throws IOException, InterruptedException  {
        String baseURL = "https://forums.spacebattles.com/threads/the-coffin-of-roboute-and-his-20-sisters-canon-guilliman-peggy-sue-into-female-primarchs-au.1132597/reader/";
        //int pageNum = 1;
        ...
        for (Element element : elements) {
			//for (Element element : elements) {
			   if (element.tagName().equals("div")) {
					filteredContent.append(sanitizeHtmlWithPandocModelB(element));//sanitizeHtmlForLatex(element.text())); // Preserve HTML structure
				   //foo(element);
...
public static String sanitizeHtmlWithPandocModelB(Element divElement) {
    ...
    try {
        ...
        BufferedReader stdoutReader = new BufferedReader(
                                      new InputStreamReader(pandocProcess.getInputStream()));
    ...
    private static void saveStoryToFile(String storyContent) throws IOException {
		...
		try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("story.tex", true), StandardCharsets.UTF_8))) {
		...
		writer.write("\\title{The Coffin of Roboute and his 20 sisters (Canon Guilliman Peggy Sue into Female-Primarchs AU)}\n");
